...
$(function(){
   ...
   $('#contact').on('submit' , function(e){
    ...
    // Supprimer les differentes erreurs
    $('#contact .has-error').removeClass('has-error');
    $('#contact .text-danger' ).remove();
    //-Declarer les variables à verifier
    var nom    = $('#nom');
    var prenom = $('#prenom');
    var email  = $('#email');
    var tel    = $('#tel');
    ...
    //Verification de chaque champ
      ...
      if(nom.val().length === 0){ // si le nom n'est pas rempli=(si le champ input 'nom' est égal à 0 caracteres)
          nom.parent().addClass('has-error'); //je rajoute une classe 'has-error'de bootstrap au parent de la variable "nom" (classe #nom) qui me surligne en rouge le champ à la validation
            $('<p class="text-danger">N\'oubliez pas de saisir votre nom</p>').appendTo(nom.parent());// j ajoute un paragraphe avec une classe "text-danger" bootstrap dans le parent de la classe nom
        } else{
        ...
        if(!validateEmail(email.val())){
            ...
            $('<p class="text-danger">Verifier votre email</p>').appendTo(email.parent());
        } else{
